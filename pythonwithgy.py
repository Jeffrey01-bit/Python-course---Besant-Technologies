...
Tops.pack(side=TOP)
Bottoms = Frame(root,width = 1600,height=50)
Bottoms.pack(side=BOTTOM)

f1 = Frame(root,width = 900,height=600)
f1.pack(side=LEFT)

f2 = Frame(root ,bg="pink",width = 700,height=600)
f2.pack(side=RIGHT)
...
lblinfo_bt = Label(Bottoms,font=('aria',12,'bold'),text="ContactUs : 9176895575",fg="steel blue",bd=10,anchor='w')
lblinfo_bt.grid(row=0,column=0)
lblinfo_bt = Label(Bottoms,font=('aria',12,'bold'),text="Email : test@gmail",fg="steel blue",bd=10,anchor='w')
lblinfo_bt.grid(row=1,column=0)
lblinfo_bt = Label(Bottoms,font=('aria',12,'bold'),text="Address : Besant Tech, Tambaram",fg="steel blue",bd=10,anchor='w')
lblinfo_bt.grid(row=2,column=0)
...
def totalcalculation():
    a = rand.get()
    b = float(Fries.get())
    c = float(Largefries.get())
    d = float(Burger.get())
    e = float(Filet.get())
    f = float(Cheese_burger.get())
    g = float(Drinks.get())
    temptotal = (b*25)+(c*40)+(d*35)+(e*50)+(f*30)+(g*35)
    subtotal = temptotal
    servicecharges = temptotal*0.05
    taxcharges = temptotal*0.10
    finaltotal = subtotal + servicecharges + taxcharges
    cost.set(finaltotal)
    Service_Charge.set(servicecharges)
    Tax.set(taxcharges)
    Subtotal.set(subtotal)
    Total.set(finaltotal)
    with open("bill.csv","a",newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow([f"Order No = {a}"])
        writer.writerow([f"Fries = {b}"])
        writer.writerow([f"Lunch Meal = {c}"])
        writer.writerow([f"Burger Meal = {d}"])
        writer.writerow([f"Pizza Meal = {e}"])
        writer.writerow([f"Cheese Burger = {f}"])
        writer.writerow([f"Drinks = {g}"])
        writer.writerow([f"Service Charge = {servicecharges}"])
        writer.writerow([f"Tax = {taxcharges}"])
        writer.writerow([f"Total = {finaltotal}"])
        writer.writerow(["----------------------"])

btnTotal=Button(f1,padx=16,pady=8, bd=10 ,fg="black",font=('ariel' ,16,'bold'),width=10, text="TOTAL", bg="powder blue", command=totalcalculation)
...
btnreset=Button(f1,padx=16,pady=8, bd=10 ,fg="black",font=('ariel' ,16,'bold'),width=10, text="RESET", bg="powder blue", command=resetvalue)
btnreset.grid(row=7, column=2)

def closeapp():
    root.destroy()

btnexit=Button(f1,padx=16,pady=8, bd=10 ,fg="black",font=('ariel' ,16,'bold'),width=10, text="EXIT", bg="powder blue", command=closeapp)
...
status = Label(f2, font=('aria', 15, 'bold'), width=16, text="-By Jeffrey", bd=2)
